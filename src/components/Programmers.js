...
export default function Programmers() {
  ...
  const [idOfProgrammer, setIdOfProgrammer] = useState(null);
  ...
  const getNameOfFeatured = () => {
    ...
    return programmersList[idOfProgrammer-1].name;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: idOfProgrammer ? 'gold' : 'royalblue'// 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmersList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setIdOfProgrammer(dev.id)/* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
